function [beta] = TVPSVQR(y,X,nsave,nburn,nthin,quant,tvp_reg,sv_reg,prior)
...
for irep = 1:(nsave+nburn)
    ...
    z  = min(1./random('InverseGaussian',k1,k2,T,n_q),1e+6); % Sample z from Inverse Gaussian
    ...
    for q = 1:n_q
        ...
        betaD(:,q) = randn_gibbs(y_tilde(:,q),xnew,miu(:,q),Q(:,q),T);  % sample regression coefficients
        if tvp_reg == 1
        ...
        elseif prior == 3
            [Q(:,q),~,miu(:,q),lambda(:,q),tau(:,q)] = horseshoe_prior(betaD(:,q),Tp,lambda(:,q),tau(:,q));
        end
        Q(1,q) = 10;
    end
...
beta = squeeze(mean(beta_draws,4));

% % Process quantiles for noncrossing
% Q_a = zeros(n_q,n_q,savedraw);
% for irep = 1:savedraw
%     mu    = Xfore*squeeze(beta_draws(end,:,:,irep));
%     if p == 1; mu = mu'; end
%     sigma = sigma_draws(end,:,irep);
%     Q_a(:,:,irep)   = repmat(mu,n_q,1) + repmat(sigma,n_q,1).*C;   % auxiliary quantiles, rows are p, columns are tau
% end
% Q_a = mean(Q_a,3);
% for b = [0.0000001, 0.0001, 0.001, 0.01, 0.1, 1]
%     K   = GPcov(b,quant,sigma_k);
%     W   = K'/(K + diag(sigma));
%     Q_f = sum(W'.*Q_a);
%     if sum(diff(Q_f)<0) == 0
%         break;
%     end
% end
% mu_mean = Xfore*squeeze(mean(beta_draws(end,:,:,:),4));
% if p == 1; mu_mean = mu_mean'; end
% if sum(diff(Q_f)<0) ~= 0
%     Q_f = sort(mu_mean);
% end
...
function [K] = GPcov(b,quant,sigma_k)
K = zeros(length(quant));
for i = 1:length(quant)
    for j = 1:length(quant)
        K(i,j) = sigma_k*exp(-0.5*((quant(i) - quant(j)).^2)./(b^2));
    end
end
%K = sigma_k*exp(-0.5*((quant' - quant).^2)./(b^2));
end